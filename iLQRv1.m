function [result] = iLQRv1(f, j, dt, N, x0, u0, p )
% iLQR for continous dynamics using boxQP
...
for iter = 1:iter_max
    ...
    for n = N-1:-1:1
        ...
        % du(dx) = l + L*dx
            ...
            lbound = umin - u(:,n);
            ubound = umax - u(:,n);
            QuuF = H + lambda*eye(dimU);
            [l(:,n), boxQP_result, boxQP_R, boxQP_free] = boxQP(QuuF,g,lbound,ubound, l(:,min(n+1,N-1))); 
            ...
            %K_i    = zeros(m,n);
            if any(boxQP_free)
                Lfree        = -boxQP_R\(boxQP_R'\G(boxQP_free,:));
                L(boxQP_free,:,n) = Lfree;
            end
